...
export class AIService implements ChordsAiPort {
  ...
  public async guessChords(chords: Chord[], language: 'es' | 'en') {
    const response = await this.http.post<AiResponse>(`${environment.API_GPT}/guesser`, {
      chords: chords,
      language: language,
    });
  ...
  public async getProgression(chords: Chord[], prompt: string, language: 'es' | 'en') {
    const response = await this.http.post<AiResponse>(`${environment.API_GPT}/progression`, {
      prompt,
      chords,
      language,
    });
  ...
  public async getAlternativeChords(chord: Chord) {
    const response = await this.http.post<AiResponse>(`${environment.API_GPT}/options`, { chord });
    return firstValueFrom(response);
  }
  public async getHandbookChords(chordName: string) {
    const response = await this.http.post<AiResponse>(`${environment.API_GPT}/forms`, { chordName });
    return firstValueFrom(response);
  }
}
